...
run_id="$(date +%s)-$$"
store_name="walstream-maintenance-$run_id"
bucket="walstream-maintenance-$run_id"
cleanup() {
...
cd "$project_dir"
cargo build --release --bin walstream --example maintenance_probe
AWS_ACCESS_KEY_ID=walstream AWS_SECRET_ACCESS_KEY=walstream-secret \
    ./target/release/examples/maintenance_probe \
    --bucket "$bucket" --region us-east-1 --endpoint "http://$store_ip:9000" --allow-http \
    --prefix "walkthrough-$run_id" --cluster-id maintenance "$@"
